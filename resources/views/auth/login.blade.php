...
<title>Gerbang — Masuk ke akun Anda</title>
...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  .form-side{ background:var(--bg-panel); padding:52px 48px; display:flex; flex-direction:column; justify-content:center; }
  .form-head h1{ font-family:'Fraunces', serif; font-size:28px; font-weight:500; margin-bottom:6px; }
  .form-head p{ font-size:14px; color:var(--text-muted); margin-bottom:24px; }
  .session-error{
  ...
  .social-row{ display:flex; flex-direction:column; gap:10px; margin-bottom:26px; }
  .social-btn{
  ...
  .divider{
    display:flex; align-items:center; gap:14px; margin:6px 0 24px;
  ...
  .remember input{ accent-color:var(--brass); width:14px; height:14px; }
  .forgot{ color:var(--brass); text-decoration:none; }
  .forgot:hover{ color:var(--brass-bright); }
  ...
  .signup-line{ text-align:center; font-size:13px; color:var(--text-muted); margin-top:22px; }
...
<div class="wrap">
  ...
  <div class="side">
    ...
    <div>
      <p class="side-copy">Satu gerbang, semua ruang kerja Anda.</p>
      <p class="side-sub">Masuk dengan aman menggunakan akun yang sudah Anda percaya, atau buat kunci baru khusus untuk Gerbang.</p>
    </div>
  ...
  <div class="form-side">
    ...
    @if ($errors->any() && !$errors->has('email') && !$errors->has('password') && !$errors->has('captcha_verified'))
    ...
    <form method="POST" action="{{ route('login.submit') }}" id="login-form" novalidate>
      ...
      <div class="field">
        ...
        <div class="field-input-wrap">
          ...
          <button type="button" class="toggle-pw" id="toggle-pw">lihat</button>
        </div>
        @error('password')
          <div class="error-msg">{{ $message }}</div>
        @enderror
      ...
      <button type="submit" class="submit-btn" id="submit-btn">Masuk</button>
    </form>

    <p class="signup-line">Belum punya akun? <a href="{{ url('/register') }}">Daftar sekarang</a></p>
  </div>
...
<script>
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    toggleBtn.textContent = isPw ? 'sembunyikan' : 'lihat';
  });
